...
// $Header: /share/content/gforge/freestylesearch/freestylesearch/src/gov/nih/nci/cadsr/freestylesearch/util/ACProperty.java,v 1.1 2006-06-30 13:46:47 hebell Exp $
...
public class ACProperty extends GenericAC
{
    ...
    {
        return "select zz.long_name || '\n\t" + _name + "\n\tPublic ID ' || zz.prop_id || "
        +"'\n\tVersion ' || zz.version || '\n\tContext ' || cc.name || "
        + "'\n\tWorkflow Status ' || zz.asl_name || "
        + "'\n\tRegistration Status ' || nvl(rs.registration_status, ' ') || "
        + "'\n\tScore " + score_
        + "' from sbrext.properties_view_ext zz, sbr.ac_registrations_view rs, sbr.contexts_view cc "
        + "where zz.prop_idseq = ? and cc.conte_idseq = zz.conte_idseq and rs.ac_idseq(+) = zz.prop_idseq";
    }
